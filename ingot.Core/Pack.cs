...
public class Pack
{
    ...
    private void WriteGeneratedScripts(string behaviourPackDir)
    {
        foreach (GeneratedScript script in GeneratedScripts)
        {
            string path = Path.Combine(behaviourPackDir, script.RelativePath);
            Directory.CreateDirectory(Path.GetDirectoryName(path)!);
            File.WriteAllText(path, script.Content);
            CompilerState.Info($"wrote generated script {script.RelativePath}");
        }

        string entryPath = Path.Combine(behaviourPackDir, ScriptEntry);
        Directory.CreateDirectory(Path.GetDirectoryName(entryPath)!);

        using StringWriter sw = new();
        sw.WriteLine("// autogenerated by ingot");
        foreach (KeyValuePair<string, Version> module in ScriptApiModules)
            sw.WriteLine($"import \"{module.Key}\";");
        foreach (GeneratedScript script in GeneratedScripts)
        {
            string importPath = "./" + script.RelativePath["scripts/".Length..];
            sw.WriteLine($"import \"{importPath}\";");
        }

        File.WriteAllText(entryPath, sw.ToString());
        CompilerState.Info($"wrote script entry {ScriptEntry}");
    }

    internal readonly record struct GeneratedScript(string RelativePath, string Content);
}