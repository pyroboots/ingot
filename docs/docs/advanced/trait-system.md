...
Most **item** and **block** traits come from Bedrock JSON schemas via [`TraitGeneratorV2`](https://pyroboots.github.io/ingot/api/ingot.Generators.html) in the `ingot.Generators` project. Entity traits still come from earlier Microsoft Learn-based generation. A small number of frequently-used properties (such as `minecraft:display_name`, `minecraft:friction`, `minecraft:light_emission`, etc.) are exposed as convenient virtual properties directly on the `Block` and `Item` base classes instead of requiring a trait.
...
> [!NOTE]
> Some trait properties are `abstract` (you **must** provide a value) because they have no default. Others are `virtual` (override only when you need a different value), often with defaults from the component schema when autogenerated. Regenerated traits typically use `string`, `dynamic`, or `string[]` when the schema type is unresolved.
...
Examples (current schema-generated traits):

| Trait | Minimum `FormatVersion` |
|-------|-------------------------|
| `IUseModifiers` | `1.26.30` |
| Most regenerated **block** traits (`IDestructibleByMining`, `ITick`, `IFlammable`, `IGeometry`, `ICollisionBox`, `IEntityFallOn`, `IRedstoneConsumer`, ...) | `1.26.20` |
| `IBlockPlacer`, `IDamage` | `1.26.0` |
| `IWearable`, `IFireResistant` (`IFireResistantItemComponent`), `IPiercingWeapon` (`IPiercingWeaponItemComponent`), `ISwingDuration` (`ISwingDurationItemComponent`), `IKineticWeapon` (`IKineticWeaponItemComponent`), `ISwingSounds` (`ISwingSoundsItemComponent`) | `1.21.90` |
| `IStorageWeightLimit`, `IStorageWeightModifier` | `1.21.60` |
| `ICompostable` | `1.21.50` |
| `IBundleInteraction`, `IDyeable`, `IRarity`, `IStorageItem` | `1.21.30` |
| `IDamageAbsorption`, `IDurabilitySensor` | `1.21.10` |
| `IIcon` | `1.20.60` |
| `IFood`, `IDurability`, `ICooldown`, `IDigger`, `IRecord`, `IUseAnimation`, and most other item traits | `1.20.50` |
...
`ingot.Core.Common.SharedConstructs` still has typed helpers for Bedrock [shared constructs](https://wiki.bedrock.dev/documentation/shared-constructs). Regenerated item and block traits currently emit `string`, `dynamic`, or `string[]` when the schema type cannot be resolved, so you usually assign a string:
...
Use the descriptor types in hand-written traits, vanilla block helpers, or anywhere you want the extra compile-time shape:

| Type | Role |
|------|------|
| `ItemTypeDescriptor` | Item id reference. Compiles as a bare identifier string. Implicit from `string` and `Item`. |
| `BlockTypeDescriptor` | Block id reference. Implicit from `string` and `Block`. |
| `BlockPermutationDescriptor` | Block id plus required states. |
| `ItemTagsDescriptor` | Tag matchers (`any` / `all` tags compiled to Molang). |
| `Range` | Min/max float range object (`{ "min": ..., "max": ... }`). |
| `Fraction` | Numerator/denominator fraction object. |

Related: `Identifier` supports implicit conversion to/from `string`, plus helpers such as `Identifier.Vanilla("dirt")` and `Identifier.VanillaAuxiliary("potion_type", "awkward")`.
...
|--------------|------------------|---------------|------------------------------------|
| Blocks       | `IBlockTrait`    | `Block`       | Used on `Block` and `BlockPermutation`. Vanilla description traits (`Block.BlockTraits`) are a separate API - see [Vanilla Block Traits](../block/block.md#vanilla-block-traits). |
...
| Type | Input | Purpose |
|------|-------|---------|
| `TraitGeneratorV2` | Bedrock JSON Schema from [mojang/bedrock-samples](https://github.com/mojang/bedrock-samples) | Schema-driven trait interfaces for items and blocks |

Bulk generation walks the GitHub repo tree (`metadata/json_schemas/server/item_components` or `.../block_components`), resolves `$ref`s, keeps the newest schema per component id, and writes `I{Component}.cs` files:

```bash
# default Program.cs entry point generates block traits (no token)
dotnet run --project ingot.Generators -- path/to/output/Block
```

`Program.cs` currently calls `TraitGeneratorV2.GenerateBlockTraits(outputDir)` and fetches the `1.26.20` block component schema from [mojang/bedrock-samples](https://github.com/mojang/bedrock-samples) over HTTP (no token). For items, call `GenerateItemTraits` with a GitHub token (`GITHUB_TOKEN` or `gh_api_token`, public repo read access):

```csharp
await TraitGeneratorV2.GenerateBlockTraits("ingot.Core/TraitSystem/Traits/Block");
await TraitGeneratorV2.GenerateItemTraits("ingot.Core/TraitSystem/Traits/Item", token);
```

Per-schema helpers remain available for one-off regeneration:

```csharp
string iface = TraitGeneratorV2.GenerateItemTraitFromSchema(schemaJson,
    "ingot.Core.TraitSystem.Traits.Item");
// also: GenerateBlockTraitFromSchema, GenerateTraitFromSchema(schema, ns, TraitType)
```

Unresolved schema types become `dynamic` (or `string[]` for arrays). Primitive JSON types map to `int` / `float` / `bool` / `string`. [Shared constructs](#shared-constructs) are not applied automatically by the current generator.

> [!NOTE]
> Entity traits and entity behaviour traits under `Traits/Entity/` are still the older generated set. Schema-driven entity generation is not wired up yet.
...
The trait system lets you compose block, item, and entity behavior using ordinary C# interface inheritance, plus optional `DynamicTraits` for object components without a generated interface and `Singles` for scalar single-value components. Regenerated traits typically use `string` / `dynamic` for identifiers and unresolved schema types; [shared constructs](#shared-constructs) remain available for hand-written traits. The result stays readable, refactorable, and type-safe while still producing the exact JSON format that Minecraft Bedrock expects.