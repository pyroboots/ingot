...
> [!NOTE]
> Some trait properties are `abstract` (you **must** provide a value) because they have no default. Others are `virtual` (override only when you need a different value), often with defaults from the component schema when autogenerated. Schema-driven item traits often use [shared constructs](#shared-constructs) instead of bare `string` / `dynamic`.
...
Examples (current schema-generated item traits):

| Trait | Minimum `FormatVersion` |
|-------|-------------------------|
| `IBlockPlacer`, `IDamage` | `1.26.0` |
| `IWearable`, `IFireResistant`, `IPiercingWeapon`, `ISwingDuration`, `IKineticWeapon` | `1.21.90` |
| `IIcon` | `1.21.80` |
| `IStorageItem`, `IStorageWeightLimit`, `IStorageWeightModifier` | `1.21.60` |
| `ICompostable` | `1.21.50` |
| `IBundleInteraction`, `IDyeable`, `IRarity` | `1.21.30` |
| `IDamageAbsorption` | `1.21.10` |
| `IUseModifiers`, `ISwingSounds`, `IRecord`, `IDurabilitySensor` | `1.21.0` |
| `IFood`, `IDurability`, `ICooldown`, `IDigger`, and most other item traits | `1.20.50` |
...
Schema-generated item traits (and regenerable block traits) map Bedrock [shared constructs](https://wiki.bedrock.dev/documentation/shared-constructs) to C# types under `ingot.Core.Common.SharedConstructs` instead of loose `dynamic` / `object` fields:
...
Call sites stay short because of implicit conversions:
...
ItemTypeDescriptor? IFood.UsingConvertsTo => "minecraft:bowl";
BlockTypeDescriptor IBlockPlacer.Block => "test:block_of_dense_lasagna";
// or: BlockTypeDescriptor IBlockPlacer.Block => myBlockInstance;
```
...
```bash
# personal access token with public repo read access
export GITHUB_TOKEN=ghp_...
# optional: gh_api_token is also accepted
dotnet run --project ingot.Generators -- path/to/output/Item
```

`Program.cs` currently calls `TraitGeneratorV2.GenerateItemTraits(outputDir, token)`. For blocks, call `GenerateBlockTraits` from a harness or change the entry point:

```csharp
await TraitGeneratorV2.GenerateItemTraits("ingot.Core/TraitSystem/Traits/Item", token);
await TraitGeneratorV2.GenerateBlockTraits("ingot.Core/TraitSystem/Traits/Block", token);
```
...
Known schema titles map onto [shared constructs](#shared-constructs) (`Item Descriptor` -> `ItemTypeDescriptor`, `Block Descriptor` -> `BlockTypeDescriptor`, `IntRange`/`FloatRange` -> `Range`, `Fraction` -> `Fraction`, `Reference` -> `Identifier`, and similar).
...
The trait system lets you compose block, item, and entity behavior using ordinary C# interface inheritance, plus optional `DynamicTraits` for object components without a generated interface and `Singles` for scalar single-value components. Schema-generated item traits use [shared constructs](#shared-constructs) for descriptors and ranges. The result stays readable, refactorable, and type-safe while still producing the exact JSON format that Minecraft Bedrock expects.