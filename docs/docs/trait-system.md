...
> [!NOTE]
> The `CompilerState` system emits warnings when values are null/empty, when block states exceed 16 values, when a trait is used on the wrong content type, and similar author mistakes. A full log is written to `{outputDir}/ingot.log` when using `Pack.Compile(..., verbose: true)` (the default).
